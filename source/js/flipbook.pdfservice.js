...
FLIPBOOK.PdfService = function (pdfDocument, model, options) {
  ...
  model.on('turnpagecomplete', function(e){
    self.rightIndex = e.rightIndex
    self.clearCanvasBuffer()
  })

  model.on('lightboxclose', function () {
  ...
  this.renderPage = function (page, size, callback) {
    ...
    var canvas = self.getCanvas(size)
    canvas.size = size
